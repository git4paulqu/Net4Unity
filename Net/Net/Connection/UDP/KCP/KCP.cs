    ...
    public class KCP
    {
        ...
        void ParseData(Segment newseg)
        {
            ...
            while (rcv_buf_.Count > 0)
            {
                ...
                {
                    Log(int.MaxValue, "parse error, sn:{0}, rev_net:{1}, nrcv_que_:{2}, rcv_wnd_:{3}", seg.sn, rcv_nxt_, nrcv_que_, nrcv_que_);
                    break;
                }
            }
        }
        ...
        {
            string message = string.Format("[KCP-{0}] {1} {2}", user_.ToString(), mask, String.Format(format, args));
